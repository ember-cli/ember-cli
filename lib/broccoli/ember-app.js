...
const calcAddonChain = require('../utilities/calc-addon-chain');
  ...
  _checkEmberCliBabel(addons, result, roots) {
    ...
    if (babelInstance) {
      ...
      if (semver.lt(version, '6.0.0')) {
        let addonChainString = calcAddonChain(babelInstance);
        if (!roots[addonChainString]) {
          roots[addonChainString] = true;
          this.project.ui.writeDeprecateLine(`ember-cli-babel 5.x has been deprecated. Please upgrade to at least ember-cli-babel 6.6. Version ${version} located: ${addonChainString}`);
        }
      }
    }
