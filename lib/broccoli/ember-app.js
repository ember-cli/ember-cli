...
var debug         = require('debug')('ember-cli:ember-app');
...
EmberApp.prototype._notifyAddonIncluded = function() {
  debug('notifyAddonIncluded');
  this.initializeAddons();
  this.project.addons = this.project.addons.filter(function(addon) {
    addon.app = this;

    if (typeof addon.isEnabled === 'function' &&  addon.isEnabled()) {
      debug('notifyAddonIncluded: %s', addon.name);

      if (typeof addon.included === 'function') {
        debug('- included!');
        addon.included(this);
      } else {
        debug('- no included function');
      }
...
EmberApp.prototype.addonTreesFor = function(type) {
  return this.project.addons.map(function(addon) {
    debug('es treeFor(%s)', addon.name, type);
    if (typeof addon.treeFor === 'function') {
      return addon.treeFor(type);
    } else {
      debug('- %s has no treeFor... skipping', addon.name);
    }
  }, this).filter(Boolean);
};
...
EmberApp.prototype.addonPostprocessTree = function(type, tree) {
  ...
  this.project.addons.forEach(function(addon) {
    debug('%s postprocessTree(%s)', addon.name, type);
    if (typeof addon.postprocessTree === 'function') {
      workingTree = addon.postprocessTree(type, workingTree);
    } else {
      debug('- %s has no postprocessTree(%s), ... skipping', addon.name, type);
    }
  });
