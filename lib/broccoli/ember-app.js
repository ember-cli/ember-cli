...
var privateContentHooks = ['app-config'];
var deprecatedContentHooks = ['app-prefix', 'app-suffix', 'vendor-prefix', 'vendor-suffix', 'config-module'];
...
EmberApp.prototype.javascript = function() {
  ...
  appJs = this._concatFiles(appJs, {
    ...
    footerFiles: [
      ...
      'vendor/ember-cli/modules-from-meta.js',
...
EmberApp.prototype.contentFor = function(config, match, type) {
  ...
  switch (type) {
    ...
    case 'app-config':       this._contentForAppConfig(content, config);    break;
  ...
  // Don't leak private content hook types.
  if (privateContentHooks.indexOf(type) === -1) {
    content = this.project.addons.reduce(function(content, addon) {
      var addonContent = addon.contentFor ? addon.contentFor(type, config, content) : null;
      if (addonContent) {
        // If attempting to use a deprecated content hook, provide a message.
        deprecate(
          'The `' + type + '` hook used in ' + addon.name + ' is deprecated. ' +
          'The addon should generate a module and have consumers `require` it.',
          deprecatedContentHooks.indexOf(type) === -1
        );
...
EmberApp.prototype._contentForHead = function(content, config) {
  ...
  if (this.options.storeConfigInMeta) {
    content.push('<meta name="' + config.modulePrefix + '/config/environment" ' +
                 'data-module="true" content="' + escape(JSON.stringify(config)) + '" />');
  }
};

/**
  @private
  @method _contentForAppConfig
  @param {Array} content
  @param {Object} config
*/
EmberApp.prototype._contentForAppConfig = function(content, config) {
  if (!this.options.storeConfigInMeta) {
...
EmberApp.prototype._contentForAppBoot = function(content, config) {
  content.push('require(\'~ember-cli/config-modules\');');
