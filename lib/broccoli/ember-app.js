...
function EmberApp(options) {
  ...
  this.tests   = options.hasOwnProperty('tests')   ? options.tests   : !this.isProduction;

  if (process.env.EMBER_CLI_TEST_COMMAND) {
    this.tests = true;
  }

  this.isTest = this.tests;

  this.hinting = options.hasOwnProperty('hinting') ? options.hinting : !this.isProduction;
...
EmberApp.prototype.javascript = function() {
  ...
  if (this.isProduction && this.options.minifyJS.enabled === true) {
...
EmberApp.prototype.styles = function() {
  ...
  if (this.isProduction && this.options.minifyCSS.enabled === true) {
