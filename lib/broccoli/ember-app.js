...
function EmberApp(options) {
  ...
  this.cleanCSSconfig = options.cleanCSSconfig || {};
...
EmberApp.prototype.appAndDependencies = memoize(function() {
  ...
  if (this.tests) {
    ...
    if (this.hinting) {
      var appAndTests = mergeTrees([app, tests]);

      var validatedJS = validateES6(appAndTests, {
        whitelist: this.importWhitelist
      });
      sourceTrees.push(validatedJS);

      var jshintedJS = jshintTrees(appAndTests, {
        destFile: this.name + '/tests/jshint-test.js'
      });
      sourceTrees.push(jshintedJS);
    }
  }
...
EmberApp.prototype.styles = memoize(function() {
  // compile things like LESS or SASS
  var cssTree = preprocessCss(this.appAndDependencies(), this.name + '/styles', '/assets');

  // minify compiled CSS in production
  if (this.env === 'production') {
    cssTree = cleanCSS(cssTree, this.cleanCSSconfig);
  }
