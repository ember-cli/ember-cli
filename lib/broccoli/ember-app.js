...
EmberApp.prototype.appAndDependencies = memoize(function() {
  ...
  var options = this.options.jsOptions;
  var preprocessedApp = preprocessJs(appAndTemplates, '/', this.name, options);
...
EmberApp.prototype.javascript = memoize(function() {
  ...
  if (this.env === 'production' && this.options.minifyJS.enabled === true) {
    ...
    return uglifyJavaScript(es6, options);
  } else {
    return es6;
  }
});
...
EmberApp.prototype.styles = memoize(function() {
  ...
  var processedStyles = preprocessCss(stylesAndVendor, this.trees.styles, '/assets', options);
