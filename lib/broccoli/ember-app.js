  ...
  _legacyAddonCompile(type, outputDir, _options) {
    ...
    let addonTrees = addonBundles.map(addonBundle => {
      ...
      if (!options.amdFunnelDisabled) {
        ...
        precompiledSource = new Funnel(precompiledSource, {
          exclude: [relativePath => {
            let [inputPath] = precompiledSource.inputPaths;
            let absolutePath = path.resolve(inputPath, relativePath);
            // if `exclude` was promise-aware,
            // we could make this faster
            let string = fs.readFileSync(absolutePath, 'utf8');
            let isAmd = AmdRegex.test(string);
            if (isAmd && !hasAlreadyPrintedAmdDeprecation) {
