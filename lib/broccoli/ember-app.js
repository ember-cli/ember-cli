...
class EmberApp {
  ...
  */
   ...
   */
  lintTestTrees() {
    let testSupportTree = this._mergeTrees(this.addonTreesFor('test-support'));

    return [
      this.lintTestTree('app', this._filterAppTree()),
      this.lintTestTree('tests', this.trees.tests),
      this.lintTestTree('templates', this._templatesTree()),
      this.lintTestTree('test-support', testSupportTree),
    ].filter(Boolean);
  }

  /**
    Lints the given tree, handling proper relocation of the files and processing of the modules. Used by `lintTestTrees`.
    ...
    @return {Tree}
   */
  lintTestTree(treeName, tree) {
    let lintedTree = this.addonLintTree(treeName, tree);
    let relocatedLintedTree = new Funnel(lintedTree, {
      srcDir: '/',
      destDir: `${this.name}/tests/`,
      annotation: `Funnel (lint ${treeName})`,
    });
    let processedLintedTree = processModulesOnly(relocatedLintedTree, `Babel: lintTree(${treeName})`);

    return processedLintedTree;
  }
