...
class EmberApp {
  ...
  constructor(defaults, options) {
    ...
    this._defaultPackager = new DefaultPackager({
      ...
      minify: {
        css: {
          enabled: this.options.minifyCSS.enabled,
          options: this.options.minifyCSS.options,
        },
      },
      distPaths: {
        testJs: this.options.outputPaths.tests.js,
  ...
  */
  ...
  /*
   * Gather MU `src` directory and return as a tree.
   *
   * @private
   * @method src
   * @return {BroccoliTree}
  */
  src() {
    ...
    if (!this._cachedUnpackagedSrcTree) {
      this._cachedUnpackagedSrcTree = new Funnel(this.trees.src, {
        destDir: 'src',
      });
    }

    return this._cachedUnpackagedSrcTree;
  }

  /**
    @private
    @method _processSrcTree
    @return
  */
  _processSrcTree() {
    if (!experiments.MODULE_UNIFICATION || !this.trees.src) {
    ...
    let srcAfterPreprocessTreeHook = this.addonPreprocessTree('src', this.src());
  ...
  /*
   * Gather templates from the app and addons and return as
   * a single tree.
   *
   * @private
   * @method templates
  ...
  */
  templates() {
    if (!this._cachedUnpackagedTemplatesTree) {
      ...
      this._cachedUnpackagedTemplatesTree = mergeTrees([
        addonTemplates,
        this._templatesTree(),
      ], {
    ...
    return this._cachedUnpackagedTemplatesTree;
  }

  /**
    @private
    @method _processTemplatesTree
    @return {BroccoliTree}
  */
  _processTemplatesTree() {
    let templates = this.addonPreprocessTree('template', this.templates());
  ...
  */
  ...
  addonTree() {
    if (!this._cachedAddonTree) {
      this._cachedAddonTree = this._addonTree('addon', 'addon-modules');
    }
  ...
  addonSrcTree() {
    if (!this._cachedAddonSrcTree) {
      this._cachedAddonSrcTree = this._addonTree('src', 'addon-modules', {
  ...
  */
  ...
  _processApp(trees) {
    let app = this.addonPreprocessTree('js', mergeTrees(trees, {
    ...
    return this.addonPostprocessTree('js', preprocessedApp);
  }

  getTests() {
    let addonTrees = this.addonTreesFor('test-support');
    return mergeTrees(addonTrees.concat(this.trees.tests), {
      overwrite: true,
      annotation: 'TreeMerger (tests)',
    });
  }

  _processTests(mergedTests) {
    let coreTestTree = this._defaultPackager.packageTests(mergedTests);

    let appTestTrees = [].concat(
      this.hinting && this.getLintTrees(),
      this.getEmberCLITree(),
    ...
    let appTestTree = this._defaultPackager.packageAppTests(appTestTrees);
  ...
  getAppLintTree() {
    ...
    let appTree = new Funnel(this.trees.app, {
      exclude: excludePatterns,
      annotation: 'Funnel: Filtered App',
    });

    let lintedApp = this.addonLintTree('app', appTree);
    return processModulesOnly(new Funnel(lintedApp, {
      srcDir: '/',
  ...
  getSrcLintTree() {
    ...
    return processModulesOnly(new Funnel(lintedSrc, {
      srcDir: '/',
  ...
  getTestsLintTree() {
    ...
    return processModulesOnly(new Funnel(lintedTests, {
      srcDir: '/',
      destDir: `${this.name}/tests/`,
  ...
  getTemplatesLintTree() {
    let lintedTemplates = this.addonLintTree('templates', this._templatesTree());

    return processModulesOnly(new Funnel(lintedTemplates, {
      srcDir: '/',
      destDir: `${this.name}/tests/`,
  ...
  getLintTrees() {
    ...
    if (this.trees.app) {
      lintTrees.push(this.getAppLintTree());
    }

    if (experiments.MODULE_UNIFICATION && this.trees.src) {
      lintTrees.push(this.getSrcLintTree());
    }

    return [
      this.getTestsLintTree(),
      this.getTemplatesLintTree(),
    ].concat(lintTrees);
  }
  ...
  _processStyles(tree) {
    if (!this._cachedStylesTree) {
      ...
      let stylesAndVendor = this.addonPreprocessTree('css', tree);

      let preprocessedStyles = preprocessCss(stylesAndVendor, '/app/styles', '/assets', options);

      let vendorStyles = this._defaultPackager.packageStyles(stylesAndVendor);

      vendorStyles = this.addonPreprocessTree('css', vendorStyles);
  ...
  */
  styles() {
    if (!this._cachedUnpackagedStylesTree) {
      ...
      this._cachedUnpackagedStylesTree = mergeTrees(trees, {
        annotation: 'TreeMerger (stylesAndVendor)',
        overwrite: true,
      });
    }

    return this._cachedUnpackagedStylesTree;
  }
  ...
  testFiles(coreTestTree) {
    ...
    let external = this.getExternalTree();
    let emberCLITree = this.getEmberCLITree();
  ...
  */
  ...
  toArray() {
    ...
    let config = this._defaultPackager.packageConfig();
    let templates = this._processTemplatesTree();

    let srcTree = this._processSrcTree();
    let trees = [this.getAppTree(), srcTree, templates].filter(Boolean);
    let external = this.getExternalTree();
    let emberCLITree = this.getEmberCLITree();

    let applicationJs = mergeTrees([
      external,
      this._processApp(trees),
      config,
      emberCLITree,
    ], {
      overwrite: true,
    ...
    let stylesTree = this._processStyles(this.styles());
    let testsTree = mergeTrees([
      this.testIndex(),
      this._processTests(this._debugTree(this.getTests(), 'app-tests')),
    ], {
      overwrite: true,
      annotation: 'Tests',
    });

    let unpackagedTests = new Funnel(mergeTrees([
      this.testIndex(),
      this.getTests(),
      this.addonTestSupportTree(),
    ].concat(this.getLintTrees()), {
      overwrite: true,
    }), {
      destDir: 'tests',
    });
    let unpackagedStyles = new Funnel(this.styles(), {
      destDir: `${this.name}/styles`,
    });
    let unpackagedPublicTree = new Funnel(publicTree, {
      destDir: 'public',
    });
    let unpackagedDist = new Funnel(mergeTrees([
      this.getAppTree(),
      ...
      unpackagedStyles,
      unpackagedTests,
      unpackagedPublicTree,
    ].filter(Boolean), {
      overwrite: true,
      annotation: 'Unpackaged Dist',
    }), {
      destDir: 'unpackaged-dist',
    });
    /*
    if (experiments.PACKAGER) {
      if (this.options.package !== undefined && typeof this.options.package === 'function') {
        return this.package(unpackagedDist).bind(this);
      }
    }
    */
    ...
    /*
      let applicationJsTree = mergeTrees([
        external,
        this._processApp(trees),
        config,
        emberCLITree,
      ...
      return this._defaultPackager.package({
        index: indexTree,
        app: {
          main: this.getAppTree(),
          external: this.getExternalTree()
          src: this.src(),
          cli: this.getEmberCLITree(),
