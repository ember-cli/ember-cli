...
class EmberApp {
  ...
  */
  index() {
    ...
    return new ConfigReplace(index, this.assembler.getConfigTree(), {
  ...
  */
  ...
  /**
    ...
    @return
  */
  _mergeApp(addonAppTrees, appTree) {
    let appTrees = addonAppTrees.concat(appTree).filter(Boolean);
  ...
  */
  ...
  _mergeTemplates(addonTemplatesTrees, appTemplatesTree) {
    let mergedTemplates = this._mergeTrees(addonTemplatesTrees, {
      overwrite: true,
  ...
  */
  ...
  _mergeAddonTrees(trees) {
    if (!this._cachedAddonTree) {
      let combinedAddonTree = this._mergeTrees(trees, {
        overwrite: true,
  ...
  /**
    ...
    @return
  */
  _processedVendorTree(addonVendorTrees, appVendorTree) {
    if (!this._cachedVendorTree) {
      let trees = [].concat(addonVendorTrees);

      if (appVendorTree) {
        trees.push(appVendorTree);
      }
  ...
  /**
    ...
    @return
  */
  _processedExternalTree(addonTrees, addonVendorTrees, bowerTree, appVendorTree, nodeModulesTrees) {
    if (!this._cachedExternalTree) {
      let vendor = this._processedVendorTree(addonVendorTrees, appVendorTree);
      let addons = this._mergeAddonTrees(addonTrees);

      let trees = [vendor].concat(addons);
      if (bowerTree) {
        trees.unshift(bowerTree);
      }

      trees = nodeModulesTrees.concat(trees);
  ...
  */
  _processedEmberCLITree() {
    if (!this._cachedEmberCLITree) {
      ...
      const dir = new UnwatchedDir(__dirname);
      let emberCLITree = new ConfigReplace(dir, this.assembler.getConfigTree(), {
        configPath: path.join(this.name, 'config', 'environments', `${this.env}.json`),
        files,
  ...
  */
  appAndDependencies() {
    let config = this.assembler.getConfigTree();
    let appTemplatesTree = this.assembler.getAppTemplatesTree();
    let addonTemplatesTrees = this.assembler.getAddonTemplatesTrees();
    let addonAppTrees = this.assembler.getAddonAppTrees();
    let appTree = this.assembler.getAppTree();

    let mainAppTree = this._mergeApp(addonAppTrees, appTree);
    let templates = this._mergeTemplates(addonTemplatesTrees, appTemplatesTree);
    let addonTrees = this.assembler.getAddonTrees();
    let addonVendorTrees = this.assembler.getAddonVendorTrees();
    let nodeModulesTrees = this.assembler.getNodeModulesTrees();
    let bowerTree = this._processedBowerTree();

    // this.assembler.createFSGraph([
    //   config, appTemplatesTree, addonTemplatesTrees, addonAppTrees
    // ]);
    ...
    let trees = [mainAppTree, srcTree, templates].filter(Boolean);
    ...
    return new BroccoliDebug(this._mergeTrees(sourceTrees, {
      overwrite: true,
      annotation: 'TreeMerger (appAndDependencies)',
    }), 'assembler:app-and-deps-tree');
  }
  ...
  */
  ...
  lintTestTrees() {
    ...
    let lintedTemplates = this.addonLintTree('templates', this.assembler.getAppTemplatesTree());
  ...
  */
  styles() {
    if (!this._cachedStylesTree) {
      ...
      let addonStylesTrees = this.addonTreesFor('styles');
      let addonTrees = this.assembler.getAddonTrees();
      let addonVendorTrees = this.assembler.getAddonVendorTrees();
      let bowerTree = this._processedBowerTree();
      let nodeModulesTrees = this.assembler.getNodeModulesTrees();
      ...
      let trees = [external].concat(addonStylesTrees, styles);
  ...
  */
  ...
  import(asset, options) {
    ...
    if (match !== null) {
      ...
      this.assembler._nodeModules.set(_path, { name, path: _path });
    }
