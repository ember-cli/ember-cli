...
function mergeTrees(_trees, options) {
  var trees = _trees.filter(Boolean);
  if (trees.length === 1) {
    return trees[0];
  } else {
    return _mergeTrees(trees, options);
  }
}
...
EmberApp.prototype.addonTreesFor = function(type) {
  ...
  return this.project.addons.map(function(addon) {
    if (addon.treeFor) {
			if (['addon'].indexOf(treeName) !== -1) {
				var treeName = type;
				var key = addon.pkg.name + '@' + addon.pkg.version + '#' + treeName;
				console.log(key);
				var hasBeenBuilt = host._treeForState[key];
				if (hasBeenBuilt) {
					console.log('hit:', key);
					return;
				}
				console.log('miss:', key);
				host._treeForState[key] = true;
			}
...
EmberApp.prototype._templatesTree = function() {
  if ('_cachedTemplateTree' in this) {
...
EmberApp.prototype._processedAppTree = function() {
  var addonTrees = this.addonTreesFor('app');
	var merge = addonTrees.concat(this._filterAppTree()).filter(Boolean);

	if (merge.length === 0) { return null; }

  var mergedApp = mergeTrees(merge, {
    overwrite: true,
...
EmberApp.prototype._processedTemplatesTree = function() {
	...
	var mergedTemplates, addonTemplates;

	if (addonTrees && addonTrees.length > 0) {
		mergedTemplates = mergeTrees(addonTrees, {
			overwrite: true,
			annotation: 'TreeMerger (templates)'
		});

		addonTemplates = new Funnel(mergedTemplates, {
			srcDir: '/',
			destDir: this.name + '/templates',
	...
	var combinedTemplates = mergeTrees(templates, {
...
EmberApp.prototype._processedTestsTree = function() {
	...
	var merge = addonTrees.concat(this.trees.tests).filter(Boolean);

	if (merge.length === 0) { return null; }

	var mergedTests = mergeTrees(merge, {
		overwrite: true,
...
EmberApp.prototype._addonTree = function _addonTree() {
  if (this._cachedAddonTree) {
	...
	var addonTrees = this.addonTreesFor('addon').filter(Boolean);

	if (addonTrees.length === 0) { return null; }

  var addonTree = mergeTrees(addonTrees, {
    overwrite: true,
    annotation: 'TreeMerger (addons)'
  });

  var addonES6 = new Funnel(addonTree, {
    srcDir: 'modules',
    allowEmpty: true,
  ...
  return this._cachedAddonTree = [
    this._concatFiles(transpiledAddonTree, {
      inputFiles: ['**/*.css'],
...
EmberApp.prototype.lintTestTrees = function() {
	...
	return [lintedApp, lintedTests, lintedTemplates].filter(Boolean);
};
