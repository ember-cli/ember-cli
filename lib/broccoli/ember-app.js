...
class EmberApp {
  ...
  constructor(defaults, options) {
    ...
    if (strictESModules(this.project)) {
      this.import('vendor/ember-cli/systemjs-extension.js', {
        prepend: true,
      });
      this.import('node_modules/systemjs/dist/extras/named-register.js', {
        prepend: true,
        resolveFrom: __dirname,
      });
      this.import('node_modules/systemjs/dist/system.js', {
        prepend: true,
        resolveFrom: __dirname,
      });
    }
    ...
    this._defaultPackager.strictESModulesSystem = strictESModules(this.project);
  ...
  */
  _initOptions(options) {
    ...
    if (emberCLIBabelInstance) {
      ...
      if (strictESModules(this.project)) {
        detectedDefaultOptions['ember-cli-babel'].compileModules = false;
        // when using compileModules:false you cannot rely on ember-cli-babel to
        // automatically disable the modules API polyfill based on the ember
        // version being new enough. You need to explicitly disable it.
        detectedDefaultOptions['ember-cli-babel'].disableEmberModulesAPIPolyfill = true;

        // when using compileModules:false, ember-cli-babel will insist on using
        // the `Ember` global to implement the debug-macros, which definitely
        // won't work since Ember 4.0. There is an incorrect comment in its
        // source claiming we can use disableDebugTooling to work around this,
        // but that kills all the debug tooling, forcing us to reimplment it.
        detectedDefaultOptions['ember-cli-babel'].disableDebugTooling = true;
      } else {
  ...
  */
  _notifyAddonIncluded() {
    ...
    let system = strictESModules(this.project);
    if (system) {
      let eai = this.project.addons.find((a) => a.name === 'ember-auto-import');
      if (!eai) {
        throw new Error(`ember-auto-import must be present to use the strict-es-modules optional feature`);
      }
      if (typeof eai.enableStrictESModules !== 'function') {
        throw new Error(`you need a newer version of ember-auto-import to use the strict-es-modules optional feature`);
      }
      eai.enableStrictESModules(system);
    }
  }
...
const strictESModulesKeys = new WeakMap();

function strictESModules(project) {
  if (!project.optionalFeatures.isFeatureEnabled('strict-es-modules')) {
    return undefined;
  }
  let key = strictESModulesKeys.get(project);
  if (!key) {
    let hash = require('crypto').createHash('sha1');
    hash.update(`${project.name}: This is private API on purpose, I will deliberately break your app if you use it.`);
    key = `_s${hash.digest('hex').slice(0, 4)}`;
    strictESModulesKeys.set(project, key);
  }
  return key;
}
