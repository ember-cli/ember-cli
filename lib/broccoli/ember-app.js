...
EmberApp.prototype.testFiles = memoize(function() {
  var testJs = concatFiles(unwatchedTree('vendor'), {
    inputFiles: this.legacyTestFilesToAppend,
    outputFile: '/assets/test-vendor.js'
  });

  var testCss = concatFiles(unwatchedTree('vendor'), {
...
EmberApp.prototype.import = function(asset, options) {
  ...
  var isJs = isType(assetPath, 'js');
  if (isJs && typeof options === 'object' && typeof options.exports  === 'undefined') {
  ...
  var nonVendoredAssetPath = path.join(subdirectory, basename);
  ...
  if (isJs) {
    if(options.type === 'vendor') {
      ...
      this.legacyTestFilesToAppend.push(nonVendoredAssetPath)
    }
    ...
    if(options.type === 'vendor') {
      ...
      this.vendorTestStyles.push(nonVendoredAssetPath);
    }
  } else {
