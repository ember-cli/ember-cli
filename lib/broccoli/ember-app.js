...
var amdDeanonymize  = require('broccoli-amd-deanonymize');
...
EmberApp.prototype.import = function(asset, options) {
  ...
  if (isType(assetPath, 'js') && options.deanonymize) {
    assetTree = amdDeanonymize(assetTree, {name: options.name});
  }
  ...
  if (isType(assetPath, 'js')) {
    ...
    this.importWhitelist = assign(this.importWhitelist, options.availableModules || {});
