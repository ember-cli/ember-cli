...
class EmberApp {
  ...
  */
  _processedTemplatesTree() {
    let addonTrees = this.assembler.getAddonTemplatesTrees();
    let mergedTemplates = new BroccoliDebug(this._mergeTrees(addonTrees, {
      overwrite: true,
      annotation: 'TreeMerger (templates)',
    }), 'assembler:addon-templates');
    ...
    let combinedTemplates = this._mergeTrees([
      addonTemplates,
      this.assembler.getAppTemplatesTree(),
    ], {
      annotation: 'addonPreprocessTree(template)',
      overwrite: true,
    });

    let templates = new BroccoliDebug(this.addonPreprocessTree('template', combinedTemplates), 'assembler:preprocessed-addon-templates');

    return new BroccoliDebug(this.addonPostprocessTree('template', preprocessTemplates(templates, {
      registry: this.registry,
      annotation: 'TreeMerger (pod & standard templates)',
    })), 'assembler:postprocessed-addon-templates');
  }
