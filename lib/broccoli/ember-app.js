...
function EmberApp(options) {
  ...
  this.getEnvJSON = function () {
    return this.project.config(this.env).get('ENV');
  }.bind(this);
...
EmberApp.prototype.appAndDependencies = memoize(function() {
  ...
  var config = this.project.config(this.env);
  if (config.isEnabled('es3Safe')) {
  ...
  if (config.isEnabled('tests')) {
    ...
    if (config.isEnabled('hinting', 'js')) {
      ...
      var jshintedTests = jshintTrees(tests, {
        jshintrcPath: config.get('hinting', 'js', 'tests') || path.join(this.project.root,  'tests'),
...
EmberApp.prototype.styles = memoize(function() {
  ...
  var config = this.project.config(this.env);
  if (config.isEnabled('minify', 'css')) {
    var options = config.get('minify', 'css') || {};
...
EmberApp.prototype.testFiles = memoize(function() {
  ...
  var config = this.project.config(this.env);
  if (config.isEnabled('fingerprint')) {
    config.push('fingerprint', 'exclude', 'testem');
  }
