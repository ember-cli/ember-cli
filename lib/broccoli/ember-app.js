...
class EmberApp {
  ...
  _addonTree(type, outputDir, _options) {
    ...
    if (experiments.DELAYED_TRANSPILATION) {
      ...
      if (options.amdFunnelDisabled) {
        ...
        combinedAddonTree = mergeTrees(addonTrees.concat(precompiledSource), {
          overwrite: true,
  ...
  */
  ...
  toArray() {
    ...
    let fullTree = this._debugTree(mergeTrees(trees, {
      overwrite: true,
  ...
  _legacyPackager(fullTree) {
    if (experiments.DELAYED_TRANSPILATION) {
      let compiledAddonSource;

      let compiledAddons = this._compileAddonTree(new Funnel(this.addonTree(), {
        srcDir: 'addon-tree-output',
      }), false);
      let addonTestSupportFiles = this._compileAddonTree(new Funnel(this.addonTestSupportTree(), {
        srcDir: 'addon-test-support',
      }), {
        skipTemplates: true,
      });

      let addonsSrc = this._compileAddonTree(new Funnel(this.addonSrcTree(), {
        srcDir: 'addon-tree-output',
      }), {
        amdFunnelDisabled: true,
        moduleNormalizerDisabled: true,
      });
      let addonTrees = new Funnel(this._compileAddonTree(new Funnel(fullTree, {
        srcDir: 'tests/addon-test-support',
      })), {
        destDir: 'tests/addon-test-support',
      });

      compiledAddonSource = new Funnel(mergeTrees([
        compiledAddons,
        addonTestSupportFiles,
        addonsSrc,
      ]), {
        destDir: 'addon-tree-output',
      });

      fullTree = mergeTrees([fullTree, compiledAddonSource, addonTrees], {
        overwrite: true,
      });
    }
