...
function EmberApp(options) {
  ...
  // the merge above has the wrong semantics for this -- it doesn't
  // let the user override with an empty list
  if (!this.options.sourcemaps) {
    this.options.sourcemaps = isProduction ? [] : ['js'];
  }
...
EmberApp.prototype.concatFiles = function(tree, options) {
  options.sourceMapsForExtensions = this.options.sourcemaps;
...
EmberApp.prototype._processedVendorTree = function() {
  ...
  addonTrees = mergeTrees([
    this.concatFiles(addonTrees, { inputFiles: ['*.css'], outputFile: '/addons.css', allowNone: true }),
    this.concatFiles(addonTrees, { inputFiles: ['*.js'],  outputFile: '/addons.js',  allowNone: true })
...
EmberApp.prototype.javascript = function() {
  ...
  if (this.options.minifyJS.enabled === true) {
    ...
    options.enableSourcemaps = this.options.sourcemaps.indexOf('js') > -1;
