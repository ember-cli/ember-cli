...
class EmberApp {
  ...
  constructor(defaults, options) {
    ...
    this.assembler = new Assembler({
      ...
      trees: this.options.trees,
      registry: this.registry,
      amdModuleNames: this.amdModuleNames
    });
  }
  ...
  */
  index() {
    ...
    return new ConfigReplace(index, this.assembler.config(), {
      configPath: path.join(this.name, 'config', 'environments', `${this.env}.json`),
      files: [htmlName],
      patterns: this.assembler._configReplacePatterns(),
    });
  }
  ...
  */
  ...
  /**
    Returns the tree for the app and its appDependencies
  ...
  */
  appAndDependencies() {
    let appSourceTrees = this.assembler.assembleApp();

    return this._mergeTrees(appSourceTrees, {
      overwrite: true,
  ...
  */
  appTests(coreTestTree) {
    let appTestTrees = [].concat(
      ...
      this.assembler._processedEmberCLITree(),
      coreTestTree
    ).filter(Boolean);
  ...
  _concatJavascript(appJsTree) {
    let appOutputPath = this.options.outputPaths.app.js;

    return this._concatFiles(appJsTree, {
  ...
  */
  javascript() {
    ...
    function deprecate(predicate, fn, message) {
      let logDeprecation = ui.writeDeprecateLine.bind(ui);
    ...
    let applicationJs = this._mergeTrees(appSourceTrees, {
      overwrite: true,
      annotation: 'TreeMerger (appAndDependencies)',
    });
    ...
    let importFile = this.import;
    let legacyFilesToAppend = this.legacyFilesToAppend;

    deprecate(this.legacyFilesToAppend.length > 0, () => {
      legacyFilesToAppend.forEach(legacyFile => importFile(legacyFile));
    }, `Usage of EmberApp.legacyFilesToAppend is deprecated. Please use EmberApp.import instead for the following files: '${this.legacyFilesToAppend.join('\', \'')}'`);
  ...
  */
  ...
  testFiles(coreTestTree) {
    ...
    let external = this._processedExternalTree();
    let emberCLITree = this.assembler._processedEmberCLITree();
  ...
  /**
    @public
    @method appDependencies
    @return {Object} Alias to the project's appDependencies function
  */
  dependencies(pkg) {
  ...
  treeShake(appTree) {
    let appJsTree =  this._concatJavascript(appTree, {
      annotation: 'TreeMerger (appJS  & processedEmberCLITree)',
      overwrite: true,
    });

    let vendorFiles = this._concatVendorJs(appTree);

    return this._mergeTrees(vendorFiles.concat(appJsTree), {
  ...
  toArray() {
    ...
    let appPackage = this.javascript();

    /*
     This is likely a wrong place for doing tree shaking but
     for the purposes of the spike seems to be okay.

     main idea is to force clear boundaries, that is:

     javascript() => return an array of broccoli trees that are not processed but ready to be
     link() => (this step does not exist yet but we could traverse dependency graph & resolve appDependencies.
     package() => given an array of trees, put the application (& friends) together the way we want

     <insert bikeshed on how to name the hook here>
     */
    let appJs = this.treeShake(appPackage);

    sourceTrees.push(appJs);
  ...
  toTree(additionalTrees) {
    let applicationTrees = this.toArray().concat(additionalTrees || []);

    let tree = this._mergeTrees(applicationTrees, {
      overwrite: true,
