...
const dependencyExists = require('../utilities/dependency-exists');
...
class EmberApp {
  ...
  */
  _initVendorFiles() {
    ...
    let needsEmberCliShims = dependencyExists(
      this.project,
      'ember-cli-babel',
      version => semver.lt(version, '6.0.0')
    );

    if (!emberShims && !addonEmberCliShims && !bowerEmberCliShims && needsEmberCliShims) {
