...
const INTERNAL_BABEL_OPTIONS = Symbol('INTERNAL_BABEL_OPTIONS');
...
class EmberApp {
  ...
  */
  ...
  /**
     Returns the babel options to be used with our internal Babel version. This is totally independent of `options.babel`.

     @private
     @method _internalBabelOptions
     @return {Object} babel options
  */
  [INTERNAL_BABEL_OPTIONS]() {
    if (!this._cachedInternalBabelOptions) {
      const buildBabelOptionsForTargets = require('../utilities/babel-config-for-targets');
      this._cachedInternalBabelOptions = buildBabelOptionsForTargets(this.project.targets);
    }

    return this._cachedInternalBabelOptions;
  }
  ...
  javascript() {
    ...
    if (!this._addonInstalled('ember-cli-babel')) {
      appJs = new Babel(
        ...
        merge(this[INTERNAL_BABEL_OPTIONS]())
      );
    }
