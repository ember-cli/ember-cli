...
EmberApp.prototype._import = function(assetPath, options, directory, subdirectory, extension) {
  ...
  if (isType(assetPath, 'js', {registry: this.registry})) {
    ...
    if (options.type === 'vendor') {
      ...
      if (!allowImport(this.legacyTestFilesToAppend, assetPath, options)) { return; }
      if (options.prepend) {
    ...
    if (options.type === 'vendor') {
      ...
      if (!allowImport(this.vendorTestStaticStyles, assetPath, options)) { return; }
      if (options.prepend) {
...
function addOutputFile(container, assetPath, options) {
  ...
  if (!allowImport(container[outputFile], assetPath, options)) { return; }
...
function allowImport(fileList, assetPath, options) {
  if (fileList.indexOf(assetPath) !== -1) {
