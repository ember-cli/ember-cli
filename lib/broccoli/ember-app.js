...
class EmberApp {
  ...
  */
  ...
  _addonTree(type, outputDir, skipTemplates) {
    ...
    let addonTrees = addonBundles.map(addonBundle => {
      ...
      if (experiments.DELAYED_TRANSPILATION) {
        ...
        let compiledSource = this._compileAddonTree(precompiledSource, skipTemplates);
  ...
  addonTestSupportTree() {
    if (!this._cachedAddonTestSupportTree) {
      this._cachedAddonTestSupportTree = this._addonTree('addon-test-support', 'addon-test-support', true);
    }
  ...
  _addonSrcTree() {
    ...
    return new Funnel(combinedAddonTree, {
      ...
      annotation: 'Funnel: addon-tree-output src',
    });
  }

  addonSrcTree() {
    if (!this._cachedAddonSrcTree) {
      this._cachedAddonSrcTree = this._addonSrcTree();
    }
...
>>>>>>> initial WIP working version of unified addon build
  ...
  */
  styles() {
    if (!this._cachedStylesTree) {
      ...
      if (options.minifyCSS.enabled === true) {
