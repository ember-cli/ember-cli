...
function getVendorFiles(files, isMainVendorFile) {
  return {
    headerFiles: files,
    inputFiles: isMainVendorFile ? ['node_modules/**/*.js'] : [],
  ...
  packageStyles(tree) {
    if (this._cachedProcessedStyles === null) {
      ...
      for (let outputFile in this.styleOutputFiles) {
        ...
        let inputFiles = isMainVendorFile ? ['node_modules/**/*.css'] : [];
  ...
  */
  processTests(tree) {
    if (this._cachedTests === null) {
      ...
      this._cachedTests = callAddonsPostprocessTreeHook(this.project, 'test', preprocessedTests);
    }
