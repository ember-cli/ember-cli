...
const {
  ...
  preprocessTemplates,
  preprocessMinifyCss,
} = require('ember-cli-preprocess-registry/preprocessors');
...
const { configReplacePatterns } = emberAppUtils;
...
module.exports = class DefaultPackager {
  ...
  */
  applyCustomTransforms(externalTree) {
    for (let customTransformEntry of this.customTransformsMap) {
      let [transformName, transformConfig] = customTransformEntry;
  ...
  */
  processTests(tree) {
    if (this._cachedTests === null) {
      ...
      testTree = addonProcessTree(this.project, 'preprocessTree', 'test', testTree);

      testTree = new Funnel(testTree, {
        destDir: `${this.name}/tests`,
        annotation: 'Packaged Tests',
      });

      let preprocessedTests = preprocessJs(testTree, '/tests', this.name, {
        registry: this.registry,
      });
  ...
  */
  packageTestFiles(tree, coreTestTree) {
    ...
    let baseMergedTree = mergeTrees([emberCLITree, tree, coreTestTree], {
      overwrite: true,
    });
    ...
    if (this.fingerprint && this.fingerprint.exclude) {
      this.fingerprint.exclude.push('testem');
    }
