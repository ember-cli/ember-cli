...
const privateSystem = new WeakMap();
...
module.exports = class DefaultPackager {
  ...
  processIndex(tree) {
    if (this._cachedProcessedIndex === null) {
      ...
      let patterns = configReplacePatterns({
        ...
        strictESModules: this.project.optionalFeatures.isFeatureEnabled('strict-es-modules'),
      });
  ...
  processAppAndDependencies(allTrees) {
    if (this._cachedProcessedAppAndDependencies === null) {
      ...
      let merged = mergeTrees(sourceTrees, {
        overwrite: true,
        annotation: 'Processed Application and Dependencies',
      });

      merged = this._strictModuleTransform(merged, 'app');

      this._cachedProcessedAppAndDependencies = this._debugTree(merged, 'app-and-deps:final');
    }

    return this._cachedProcessedAppAndDependencies;
  }

  set strictESModulesSystem(system) {
    privateSystem.set(this, system);
  }

  /*
    The strict-es-modules optional feature causes us to set `compileModules:
    false` in ember-cli-babel. But when that is set, ember-cli-babel will not
    emit the correct debug tooling, insisting instead on implementing it in
    terms of the (removed) `Ember` global. There's no option that can force it
    to setup the tooling correctly, so all we can do is disable the default
    tooling and reconfigure it ourselves here.
  */
  _debugTooling() {
  ...
  _modulesToSystemJS(tree) {
    const BroccoliBabel = require('broccoli-babel-transpiler');

    const { getModuleId, resolvePath } = require('./resolve-path');

    return new BroccoliBabel(tree, {
      babel: {
        moduleId: true,
        getModuleId,
        plugins: [
          ...
          // transform-modules-systemjs emits a warning if you don't have this
          // one too
          require.resolve('@babel/plugin-proposal-dynamic-import'),
          [
            require.resolve('@babel/plugin-transform-modules-systemjs'),
            {
              systemGlobal: privateSystem.get(this),
            },
          ],
        ],
      },
  ...
  _strictModuleTransform(tree, which) {
    if (!this.project.optionalFeatures.isFeatureEnabled('strict-es-modules')) {
      return tree;
    }

    tree = this._debugTree(tree, `strict-modules-in:${which}`);

    let addonDir = which === 'app' ? 'addon-tree-output' : 'addon-test-support';

    return this._debugTree(
      mergeTrees(
        [
          tree,
          new Funnel(
            this._modulesToSystemJS(
              new Funnel(tree, {
                srcDir: addonDir,
              })
            ),
  ...
  packageEmberCliInternalFiles() {
    if (this._cachedEmberCliInternalTree === null) {
      ...
      let strictESModulesKey = privateSystem.get(this);
      if (strictESModulesKey) {
        patterns.push({
          match: /PRIVATE_SYSTEM_HERE/g,
          replacement: strictESModulesKey,
        });
      }
      ...
      // TODO: if storeConfigInMeta is false, this spots needs to combine both
      // the test and dev configs into one module.

      this._cachedEmberCliInternalTree = new Funnel(emberCLITree, {
        files: EMBER_CLI_FILES,
        annotation: 'Packaged Ember CLI Internal Files',
        getDestinationPath: (originalPath) => {
          switch (originalPath) {
            case 'app-config.js':
              return `${this.name}/config/environment.js`;
            case 'compat-modules.js':
              return `addon-tree-output/@ember/compat-modules/index.js`;
            default:
              return path.resolve(EMBER_CLI_INTERNAL_FILES_PATH, originalPath);
          }
        },
      });
    }
  ...
  packageTests(tree) {
    ...
    coreTestTree = this._strictModuleTransform(coreTestTree, 'tests');
  ...
  packageTestApplicationConfig() {
    if (this.project.optionalFeatures.isFeatureEnabled('strict-es-modules')) {
      // under strict-es-modules, we can't rely on smashing the dev config with
      // the tests config inside the AMD loading. Instead:
      // - when storeConfigInMeta is true we can simply do nothing and rely on
      //   dev & test to both see the correct config in their meta datgs.
      // - when storeConfigInMeta is false, we inline both dev and test configs
      //   into the same module, with embroider macros to distinguish between
      //   them.
      return;
    }
  ...
  packageApplicationTests(tree) {
    ...
    return concat(appTestTrees, {
      inputFiles: ['**/tests/**/*.js'],
      headerFiles: this.project.optionalFeatures.isFeatureEnabled('strict-es-modules')
        ? []
        : ['vendor/ember-cli/tests-prefix.js'],
      footerFiles: this.project.optionalFeatures.isFeatureEnabled('strict-es-modules')
        ? ['vendor/ember-cli/tests-suffix-es.js']
        : ['vendor/ember-cli/app-config.js', 'vendor/ember-cli/tests-suffix.js'],
  ...
  packageJavascript(tree) {
    if (this._cachedJavascript === null) {
      ...
      if (!this.scriptOutputFiles[vendorFilePath]) {
        this.scriptOutputFiles[vendorFilePath] = [];
      }
  ...
  packageApplicationJs(tree) {
    ...
    let headerFiles = this.project.optionalFeatures.isFeatureEnabled('strict-es-modules')
      ? []
      : ['vendor/ember-cli/app-prefix.js']; // useless "use strict"
    let footerFiles = this.project.optionalFeatures.isFeatureEnabled('strict-es-modules')
      ? ['vendor/ember-cli/app-boot.js']
      : ['vendor/ember-cli/app-suffix.js', 'vendor/ember-cli/app-config.js', 'vendor/ember-cli/app-boot.js'];
