  ...
  run(environment) {
    ...
    return RSVP.hash(environment).then(environment => {
      ...
      let runPromise = Promise.resolve().then(() => {
        ...
        console.log('ValidateAndRun:before');
        return command.validateAndRun(commandArgs)
          .then(e => {
            console.log('command finished', e);
            return e;
          }, e => {
            console.log('validate and run error', e);
            throw e;
          });
      }).finally(() => {
        console.log('removeHandler', commandName, onCommandInterrupt);
      ...
      console.log('assign command interruption handler');
      onCommandInterrupt = () => {
        console.log('interruption handler started', commandName);
        Promise.resolve(command.onInterrupt())
          .then(() => runPromise.then(a => {
