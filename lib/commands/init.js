...
let clone = require('ember-cli-lodash-subset').clone;
...
let validProjectName = require('../utilities/valid-project-name');
let normalizeBlueprint = require('../utilities/normalize-blueprint-option');
let mergeBlueprintOptions = require('../utilities/merge-blueprint-options');
...
module.exports = Command.extend({
  ...
  run(commandOptions, rawArgs) {
    ...
    let gitInit, npmInstall, addonInstall, bowerInstall;
    ...
    if (!commandOptions.skipNpm) {
      ...
      addonInstall = new this.tasks.AddonInstall({
        ui: this.ui,
        analytics: this.analytics,
        project: this.project,
    ...
    return installBlueprint.run(blueprintOpts)
      ...
      .then(() => {
        // skip `ember generate ember-cli-jshint` if the plugin hasn't been installed
        if (!commandOptions.skipNpm) {
