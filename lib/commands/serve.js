    ...
    return this._checkExpressPort(commandOptions).then(commandOptions => {
      if (commandOptions.proxy) {
        if (!/^(http:|https:)/.test(commandOptions.proxy)) {
          let message = `You need to include a protocol with the proxy URL.${EOL}Try --proxy http://${
            commandOptions.proxy
          }`;
      ...
      return Win.checkIfSymlinksNeedToBeEnabled(this.ui).then(() => this.runTask('Serve', commandOptions));
    });
  },
