...
module.exports = Command.extend({
  ...
  availableOptions: [
    ...
    },
    {
      ...
      aliases: ['lro'],
      description:
        'Stringified JSON object for LiveReload options. LiveReload supports a number of options for configuring websocket communication, including https, host, port, and others. See advanced example below.',
    },
  ...
  async run(commandOptions) {
    ...
    if (typeof commandOptions.liveReloadOptions === 'string') {
      try {
        commandOptions.liveReloadOptions = JSON.parse(commandOptions.liveReloadOptions);
      } catch (e) {
        let message = `--live-reload-options value should be valid serialized JSON string, got: ${commandOptions.liveReloadOptions}`;
