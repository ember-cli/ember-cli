...
module.exports = Command.extend({
  ...
  availableOptions: [
    ...
    { name: 'live-reload-port',     type: Number,                          aliases: ['lrp'],                                                   description: '[Deprecated] Now uses same port as ember app' },
  ...
  run(commandOptions) {
    if (commandOptions.liveReloadPort) {
      throw new SilentError(`Now uses same port as ember app`);
    }
  ...
  _checkExpressPort(commandOptions) {
    return getPort({ port: commandOptions.port, host: commandOptions.host }).then(foundPort => {
      ...
      commandOptions.liveReloadPort = commandOptions.port;
