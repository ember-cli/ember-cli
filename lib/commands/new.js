...
const { MSG_HELP } = require('../utilities/get-lang-arg');
...
module.exports = Command.extend({
  ...
  async run(commandOptions, rawArgs) {
    const lang = commandOptions.lang;
    const isInvalidLang =
      lang === '' ||
      lang === true ||
      (typeof lang === 'string' && lang.startsWith('-'));

    if (isInvalidLang) {
      const errorMessage = `The "--lang" option requires a valid language code value.\n\n${MSG_HELP}`;
