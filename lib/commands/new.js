...
const { DEFAULT_APP, DEFAULT_ADDON } = require('../tasks/install-blueprint');
...
module.exports = Command.extend({
  ...
  availableOptions: [
    ...
    { name: 'blueprint', type: String, default: DEFAULT_APP, aliases: ['b'] },
  ...
  run(commandOptions, rawArgs) {
    ...
    if (commandOptions.blueprint === 'addon') {
      commandOptions.blueprint = DEFAULT_ADDON;
    }
    ...
    if (!packageName) {
      message = `The \`ember ${
        this.name
      }\` command requires a name to be specified. For more details, use \`ember help\`.`;
    ...
    if (packageName === '.') {
      let blueprintName = commandOptions.blueprint === DEFAULT_APP ? 'application' : commandOptions.blueprint;
