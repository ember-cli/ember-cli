...
module.exports = Command.extend({
  ...
  _printJsonHelp: function(commandOptions, rawArgs) {
    ...
    var jsonString;
    var generateJsonCommand = this._lookupCommand('generateJson');
    var cachePath = this.project.root +
                    generateJsonCommand.cacheDirectory + '/' +
                    generateJsonCommand.cacheFileName;

    var hasCache = fs.existsSync(cachePath);

    if (rawArgs.length === 0 && hasCache) {

      jsonString = fs.readFileSync(cachePath, 'utf8');

    } else {

      jsonString = generateJsonCommand.run(commandOptions, rawArgs);

    }

    this.ui.writeLine(jsonString);
  },
