...
module.exports = class Watcher extends CoreObject {
  ...
  constructBroccoliWatcher(options) {
    let watcher = new (require('broccoli')).Watcher(this.builder, this.builder.builder.watchedSourceNodeWrappers, {
      saneOptions: options,
    });

    watcher.on('buildStart', this._setupBroccoliWatcherBuild.bind(this));
    watcher.on('buildSuccess', this.didChange.bind(this));
    watcher.start();
  ...
  didChange(results) {
    ...
    if (isExperimentEnabled('BROCCOLI_WATCHER') && !this.foobar) {
