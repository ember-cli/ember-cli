...
module.exports = class Watcher extends CoreObject {
  ...
  setupBroccoliChangeEvent() {
    ...
    if (this.verbose) {
      this.watcher.on('add', filePath => this.ui.writeLine(`file foo added ${filePath}`));
      this.watcher.on('delete', filePath => this.ui.writeLine(`file bar deleted ${filePath}`));
      this.watcher.on('change', filePath => this.ui.writeLine(`file baz changed ${filePath}`));
    }
  }
