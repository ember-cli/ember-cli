...
/**
  ...
  cli commands including options used by the CliCommandCompleter. If you
  have the current `project` available as an object, you should pass it
  to the constructor as this will speed up generation by factor 3.
  Otherwise it needs to infer its own `project` from current working
  directory.
...
function CliCommandGenerator(project) {
  ...
  this.cacheDir = '/node_modules/ember-cli/';
  this.cacheFile = 'cli-commands.json';
...
CliCommandGenerator.prototype.run = function(sync) {

  var json = this.generateJSON(this.root);
  var result = JSON.stringify(json, null, ' ');
  var pathToCache = this.project.root + this.cacheDir + this.cacheFile;

  if (sync) {
    this.openOrCreateCache(this.project.root + this.cacheDir, this.cacheFile);
    this.setupShellCompletion();

    fs.writeFileSync(pathToCache, result);
  } else {
    fs.writeFile(pathToCache, result);
  }
...
/**
  Syncronously writes cached cli command JSON into a file
...
CliCommandGenerator.prototype.runSync = function() {
...
CliCommandGenerator.prototype.openOrCreateCache = function(dirPath, fileName) {

  if (!fs.exists(dirPath + fileName)) {
    mkdirp.sync(dirPath);
    fs.openSync(dirPath + fileName, 'w+');
  }
...
CliCommandGenerator.prototype.prepareProject = function(project) {
  ...
  try {
    ...
    project = project || Project.nullProject();
  }
...
CliCommandGenerator.prototype.setupShellCompletion = function() {
  ...
  process.env.SHELL = shell || '/bin/bash';
...
CliCommandGenerator.prototype.buildCommand = function(command, parentCommands) {
  ...
  cliCommands = cliCommands.filter(function(c) { return c.name; });

  var result = new CliCommand(command);
  result.commands = cliCommands;
  return result;
};
...
CliCommandGenerator.bail = function() {
  return;
};
