...
var AddonDiscovery = CoreObject.extend({
  ...
  */
   ...
   */
  discoverFromProjectItself: function(project) {
   ...
   */
  discoverFromDependencies: function(root, nodeModulesPath, pkg, excludeDevDeps) {
    var discovery = this;
  ...
  resolvePackage: function(root, packageName) {
    try {

      var resolvedPath = resolve.sync(packageName, { basedir: root });
      var entryModulePath = fs.realpathSync(resolvedPath);
   ...
   */
  discoverInRepoAddons: function(root, pkg) {
   ...
   */
  discoverFromInternalProjectAddons: function(project) {
    var discovery = this;
  ...
  addonPackages: function(addonsList) {
    var addonPackages = {};
