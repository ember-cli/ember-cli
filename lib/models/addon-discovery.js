...
AddonDiscovery.prototype.discoverFromDependencies = function(root, pkg, excludeDevDeps) {
  ...
  var addons = Object.keys(this.dependencies(pkg, excludeDevDeps)).map(function(name) {
    if (name !== 'ember-cli') {
      ...
      if (addon) {
        console.warn('The package `' + name + '` is not a properly formatted package, we have used a fallback lookup to resolve it at `' + addonPath + '`. This is generally caused by an addon not having a `main` entry point (or `index.js`).');
        return addon;
      }
    }
  }, this).filter(Boolean);
