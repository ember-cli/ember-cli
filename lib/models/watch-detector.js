  ...
  */
  checkWatchman() {
    ...
    try {
      ...
      let isGreaterThanEqualToV3 = false;
      let canNestRoots = false;
      try {
        isGreaterThanEqualToV3 = semver.satisfies(version, '>= 3.0.0');
        canNestRoots = semver.satisfies(version, ">= 3.7.0");
      } catch (e) {
        const dateStr = version
          .replace(/^v/, "")
          .replace(/\./g, "/")
          .split("/")
          .slice(0, -1)
        ...
        // all dated versions of watchman are successors of v3
        isGreaterThanEqualToV3 = isValidDate;
        canNestRoots = isValidDate;
      }

      if (isGreaterThanEqualToV3) {
