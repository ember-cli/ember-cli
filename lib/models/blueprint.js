...
let Blueprint = CoreObject.extend({
  ...
  */
  addBowerPackagesToProject(packages, installOptions) {
    ...
    packages.forEach(pkg => (pkg.source = pkg.source || pkg.name));

    let packageNames = packages.map(pkg => pkg.name);
    let packageNamesAndVersions = packages.map(bowEpParser.compose);
...
function inRepoAddonExists(name, root) {
  ...
  return Blueprint._existsSync(addonPath);
}
