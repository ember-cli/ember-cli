...
let Blueprint = CoreObject.extend({
  ...
  */
  async _process(options, beforeHook, process, afterHook) {
    ...
    const transformed = await Promise.all(
      fileInfos.map((info) => {
        if (this.shouldConvertToJS(options, info)) {
    ...
    await Promise.all(transformed.map((info) => this._commit(info)));
  ...
  shouldConvertToJS(options, fileInfo) {
    ...
    return !(options.isTypeScriptProject || options.typescript) && isTypeScriptFile(fileInfo.inputPath);
  },

  async convertToJS(fileInfo) {
    let rendered = await fileInfo.render();

    // swc collapses whitespace between lines, which we want to preserve, so we insert markers in all the places where
    // we know we need to preserve the extra newline before running the code through swc
    rendered = rendered.replace(/\n\n/g, '/* ___NEWLINE___ */\n');

    // for some reason, swc's TypeScript syntax transform handles default class exports by re-writing
    // `export default class Foo` to `export { Foo as default }` preceded by `class Foo`. Since `export default class`
    // is idiomatic throughout Ember, we want to preserve this syntax even after swc rewrites it. To that end, we make a
    // note of whether the file's default export is a class so that we can rewrite it after the transform.
    const hasExportDefaultClass = rendered.includes('export default class');

    const transformed = await swc.transform(rendered, {
      jsc: {
        parser: {
          syntax: 'typescript',
          decorators: true,
    ...
    // Add all the whitespace between lines back by replacing all the markers we inserted earlier
    let fixed = transformed.code.replace(/\/\* ___NEWLINE___ \*\//g, '\n');

    // This entire next block is responsible for handling swc's weird re-write of default class exports. It uses recast
    // to traverse the AST and:
    // 1. find the export declaration that swc created for the default (e.g. `export { Foo as default }`)
    // 2. Note the name of the class (.e.g. `Foo`)
    // 3. Remove this export node from the AST
    // 4. Find the class declaration that corresponds to the name of the class we noted earlier
    // 5. wrap the class declaration in a default export declaration
    if (hasExportDefaultClass) {
      const ast = recast.parse(fixed);
      ...
      recast.types.visit(ast, {
        visitExportNamedDeclaration(path) {
          ...
          const defaultExport = specifiers.find((spec) => spec.exported.name === 'default');

          if (!defaultExport) {
            return false;
          }
          defaultClassName = defaultExport.local.name;

          // Remove this node from the AST now that we've gotten the class name we need, and stop traversal.
          path.replace();
          this.abort();
        },
      });
      ...
      recast.types.visit(ast, {
        visitClassDeclaration(path) {
          if (path.node.id.name !== defaultClassName) {
            return false;
          }

          const defaultExport = builders.exportDefaultDeclaration(path.node);
          path.replace(defaultExport);

          this.abort();
        },
      });

      fixed = recast.print(ast).code;
    }

    fileInfo.rendered = prettier.format(fixed, {
      singleQuote: true,
      parser: 'babel',
    });
  ...
  */
  processFiles(intoDir, templateVariables) {
    ...
    fileInfosToRemove = finishProcessingForUninstall(fileInfosToRemove, this.shouldTransformTypeScript);
  ...
  */
  processFilesForUninstall(intoDir, templateVariables) {
    ...
    return finishProcessingForUninstall(fileInfos.filter(isValidFile), this.shouldTransformTypeScript);
  },
...
function finishProcessingForUninstall(infos, shouldTransformTypeScript) {
  let validInfos = infos.filter((info) => {
    if (shouldTransformTypeScript && isTypeScriptFile(info.outputPath)) {
      const jsPath = replaceExtension(info.outputPath, '.js');
      return fs.existsSync(jsPath) || fs.existsSync(info.outputPath);
    }
    return fs.existsSync(info.outputPath);
  });
