...
let Blueprint = CoreObject.extend({
  ...
  */
  ...
  shouldConvertToJS(options, fileInfo) {
    ...
    // them the JS output
    if (options.typescript && !isTypeScriptFile(fileInfo.inputPath)) {
      console.log('should print warning line');
      this.ui.writeLine(
        chalk.yellow(
    ...
    // If it's a TypeScript file, we transform it *unless* the user has asked for TypeScript either
    // globally (by setting `isTypeScriptProject` to true) or locally (by passing the `--typescript`
    // flag when they invoked the generator).
