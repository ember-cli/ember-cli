...
Blueprint.prototype.install = function(options) {
  ...
  this.pods    = options.pods || false;
  this.podPath = options.podPath = generatePodPath();
...
Blueprint.prototype._fileMapTokens = function(options) {
  var hasPathToken = this.files().join().match(/__path__/g);
  var standardTokens = {
    __name__: function(options) {
      if (options.pods && hasPathToken) {
...
/*
  ...
  This is used primarily in the install method.

  @method generateFileMap
  @param {Object} locals
  @return {Object}
*/
Blueprint.prototype.generateFileMap = function(tokens, locals){
  var fileMapValues = values(tokens);
  var tokenValues   = fileMapValues.map(function(token) { return token(locals); });
...
Blueprint.prototype._locals = function(options) {
  ...
  var fileMapOptions = {
    ...
    blueprintName: this.name,//options.blueprintName, //this.name,
    dasherizedModuleName: stringUtils.dasherize(moduleName)
  };
  var fileMapTokens = this._fileMapTokens(fileMapOptions);
  var fileMap = this.generateFileMap(fileMapTokens, fileMapOptions);
