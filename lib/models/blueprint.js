...
let Blueprint = CoreObject.extend({
  ...
  */
  ...
  async convertToJS(fileInfo) {
    ...
    rendered = rendered
      .replace(/(\/\/.*)\n(?!\n|\s*\/\/)/g, '$1/* ___DELETE ME AND THE NEWLINE AFTER ME___ */\n')
      .replace(/\n\n/g, '/* ___NEWLINE___ */\n');

    const transformed = await babel.transformAsync(rendered, {
      plugins: [babel.createConfigItem(require('@babel/plugin-transform-typescript'))],
    });

    const fixed = transformed.code
      .replace(/\/\* ___NEWLINE___ \*\//g, '\n')
      .replace(/\/\* ___DELETE ME AND THE NEWLINE AFTER ME___ \*\/\n/g, '')
      .replace(/\n(\/\*,.*\*\/)\n/g, '$1');
