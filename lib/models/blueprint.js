...
let Blueprint = CoreObject.extend({
  ...
  */
  shouldConvertToJS(options, fileInfo) {
    ...
    // Since the `--typescript` is Boolean, ember-cli automatically generates a `--no-typescript` flag
    // as well. This means that `options.typescript` will be undefined if *nothing* is passed, but
    // will be explicitly false if `--no-typescript` is passed. We use this variable to differentiate
    // between both cases
    const noTypeScript = options.typescript === false;
    ...
    // the erroneously trigger this message every time they generate a JS blueprint even though
    // they didn't pass the flag.
    if (!noTypeScript && !isTypeScriptFile(fileInfo.inputPath)) {
      this.ui.writeLine(
        chalk.yellow(
    ...
    // invoked the generator). Since ember-cli merges `.ember-cli` and the all of the flag values into
    // one object, we could theoretically *just* have a `typescript` flag that is either passed in
    // to `generate` OR set in `.ember-cli`, but we thought the DX would be improved by differentiating
    // between what is intended to be global vs. local config. Note that, because of this behavior,
    // the user could *technically* also just set `typescript: true` in `.ember-cli` or pass
    // `--is-typescript-project` when invoking `generate`, but we don't expect this to happen often.
    // const userRequestedTypeScript = options.typescript !== false && options.isTypeScriptProject;
    let userRequestedTypeScript = !noTypeScript && (options.typescript === true || options.isTypeScriptProject);

    // If it's a TypeScript file, we transform it *unless* the user has specifically requested
    // TypeScript locally or globally (see the comment above for more info)
    return !userRequestedTypeScript && isTypeScriptFile(fileInfo.inputPath);
  },
