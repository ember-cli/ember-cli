...
var Blueprint = CoreObject.extend({
  ...
  */
  generateFileMap(fileMapVariables) {
    ...
    var tokenValues = fileMapValues.map(token => token(fileMapVariables));
  ...
  */
  addBowerPackagesToProject(packages, installOptions) {
    ...
    var packageNames = [];
    var packageNamesAndVersions = packages.map(pkg => {
...
Blueprint.list = function(options) {
  ...
  return lookupPaths.map(lookupPath => {
    ...
    var blueprints = dir(lookupPath).map(blueprintPath => {
