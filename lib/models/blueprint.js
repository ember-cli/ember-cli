...
var debug       = require('debug')('blueprint');
...
function Blueprint(blueprintPath) {
  ...
  debug('new Blueprint path: %s name: ', this.path, this.name);
}
...
Blueprint.prototype.files = function() {
  ...
  debug('Blueprint#files %s files: %o', this.name, this._files);
  return this._files;
};
...
Blueprint.prototype.srcPath = function(file) {
  var result = path.resolve(this.path, 'files', file);
  debug("%s file: %s srcPath: %s", this.name, file, result);
  return result;
};
...
Blueprint.prototype.install = function(options) {
  ...
  debug('installing: name: %s, testing: %o, pod: %o, hasPath: %o', this.name, this.testing, this.pod, this.hasPath);
...
Blueprint.prototype.processFiles = function(intoDir, templateVariables) {
  debug('processFiles, into: %s', intoDir)
