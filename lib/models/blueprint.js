...
const normalizeAddonPath = require('../utilities/normalize-addon-path');
...
let Blueprint = CoreObject.extend({
  ...
  */
  _checkInRepoAddonExists(options) {
    ...
    if (options.inRepoAddon || options.in) {
      addon = options.inRepoAddon ? findAddonByName(this.project, options.inRepoAddon) : findAddonByPath(this.project, options.in);

      if (!addon) {
        let option = options.inRepoAddon ? 'in-repo-addon' : 'in';

        throw new SilentError(`You specified the ${option} flag, but the ` +
...
function findAddonByPath(project, addonPath) {
  return project.addons.find(addon => addon.path === normalizeAddonPath(addonPath));
}
