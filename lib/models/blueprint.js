...
let Blueprint = CoreObject.extend({
  ...
  */
  async removeTypes(extension, code) {
    ...
    } else if (extension === '.gts') {
      const TEMPLATE_TAG_IDENTIFIER = '__TEMPLATE_TAG__;';
      const { parseTemplates } = require('ember-template-tag');
      const gts = parseTemplates(code);
      const templateTag = code.substring(gts[0].range[0], gts[0].range[1]);
      const codeWithStrippedTemplateTag =
        code.substring(0, gts[0].range[0]) + TEMPLATE_TAG_IDENTIFIER + code.substring(gts[0].range[1]);

      const { removeTypes } = require('remove-types');
      const transformed = await removeTypes(codeWithStrippedTemplateTag);
      const transformedWithTemplateTag = transformed.replace(TEMPLATE_TAG_IDENTIFIER, templateTag);
