...
Blueprint.prototype._actions = {
  write: function(info) {
    this.ui.writeLine('  ' + chalk.green('create') + ' ' + info.displayPath);
    if (!this.dryRun) {
  ...
  skip: function(info) {
    ...
    this.ui.writeLine('  ' + chalk.yellow(label) + ' ' + info.displayPath);
  },

  overwrite: function(info) {
    this.ui.writeLine('  ' + chalk.yellow('overwrite') + ' ' + info.displayPath);
    if (!this.dryRun) {
      return writeFile(info.outputPath, info.render());
    }
  },
  ...
  edit: function(info) {
    this.ui.writeLine('  ' + chalk.green('edited') + ' ' + info.displayPath);
  },

  remove: function(info) {
    this.ui.writeLine('  ' + chalk.red('remove') + ' ' + info.displayPath);
    if (!this.dryRun) {
...
Blueprint.prototype.install = function(options) {
  ...
  return Promise.resolve()
    ...
    .then(this.processFiles.bind(this, intoDir, locals)).map(this._commit.bind(this))
...
Blueprint.prototype.uninstall = function(options) {
  ...
  return Promise.resolve()
    .then(this.beforeUninstall.bind(this, options, locals))
    .then(this.processFilesForUninstall.bind(this, intoDir, locals)).map(this._commit.bind(this))
...
/**
  ...
  the blueprint to support addon tokens.
