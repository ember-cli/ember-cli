...
Blueprint.prototype._locals = function(options) {
  ...
  return new Promise(function(resolve) {
    return resolve(this._processPrompts(options));
  }.bind(this)).then(function (prompts) {
    return this.locals(prompts);
...
/**
  @private
  @method _processPrompts
  @param {Object} options
  @return {Promise}
*/
Blueprint.prototype._processPrompts = function (options) {
  if (this.ui.ci || options.skipPrompts) {
    return Promise.resolve(options);
  }

  var availableOptions = this.availableOptions;
  var prompts = availableOptions
    ...
    .map(function (item) {
      ...
      var props = {
        name: inflector.camelize(item.name, true),
        type: this._defaultPromptType(item),
        message: item.description,
        default: item.default
      };

      return merge(props, prompt);
    }.bind(this));

  return this.ui.prompt(prompts)
    .then(function (answers) {
      return merge(options, answers);
    });
};

/**
  @private
  @method _defaultPromptType
  @param {Object} options
  @return {String}
*/
Blueprint.prototype._defaultPromptType = function (options) {
  switch (options.type) {
    case Boolean:
    default: {
      return 'confirm';
