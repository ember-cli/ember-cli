...
var debug        = require('debug')('file-info');
...
FileInfo.prototype.confirmOverwrite = function(path) {
  ...
  return this.ui.prompt(promptOptions)
    .then(function(response) {
      debug('%s overwrite:%o', path, response.answer);
...
function FileInfo(options) {
  ...
  debug('FileInfo: %s -> %s', this.inputPath, this.outputPath);
}

FileInfo.prototype.render = function() {
  debug('render input: %s', this.inputPath);
