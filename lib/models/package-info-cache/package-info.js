...
class PackageInfo {
  ...
  addDependencies(dependencies) {
    ...
    dependencyNames.forEach(dependencyName => {
      ...
      if (!dependencyPackage && pnp) {
        logger.info('Resolving %o through Yarn PnP starting at %o', dependencyName, this.realPath);
        let pkgPath = pnp.resolveRequest(`${dependencyName}/package.json`, `${this.realPath}/`);
        let realPath = path.dirname(pkgPath);

        dependencyPackage = this.cache._readPackage(realPath, undefined, undefined, true);
      }
