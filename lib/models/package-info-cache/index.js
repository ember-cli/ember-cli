...
class PackageInfoCache {
   ...
   */
  _readPackage(packageDir, pkg, isRoot, skipReal = false) {
    ...
    let realPath = skipReal ? normalizedPackageDir : getRealDirectoryPath(normalizedPackageDir);
    ...
    if (pkgInfo.mayHaveAddons) {
      ...
      let nodeModules = this._readNodeModulesList(path.join(realPath, 'node_modules'), skipReal);
   ...
   */
  _readNodeModulesList(nodeModulesDir, skipReal = false) {
    ...
    if (skipReal) {
      realPath = normalizedNodeModulesDir;
    }
