...
function Project(root, pkg, ui, cli) {
  ...
  var instrumentation = this._instrumentation = ensureInstrumentation(cli, ui);
...
Project.prototype.getAddonsConfig = function(env, appConfig) {
  ...
  var initialConfig = _.merge({}, appConfig);
...
Project.prototype.addonCommands = function() {
  ...
  this.addons.forEach(function(addon) {
    ...
    if (total > 20 /* ms */) {
      ui.writeWarnLine('Addon[' + addon.name + ']\'s commands took: ' + total + 'ms to load, see: http://some-link.com/help');
    }
...
Project.prototype.eachAddonCommand = function(callback) {
  if (this.initializeAddons && this.addonCommands) {
    ...
    _.forOwn(addonCommands, function(commands, addonName) {
