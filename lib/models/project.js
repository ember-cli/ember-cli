...
class Project {
  ...
  */
  ...
  configWithoutCache(env) {
    ...
    if (fs.existsSync(`${configPath}.js`)) {
      ...
      const mergedConfig = _.merge(addonsConfig, appConfig);

      const deprecationCompliance = {};
      deprecationCompliance.self = this.pkg.ember.deprecationCompliance;
      deprecationCompliance.addons = {};

      this.addons.forEach((x) => {
        if (x.pkg.ember && x.pkg.ember.deprecationCompliance) {
          console.log('✅', 'deprecation compliance: ', x.name, x.pkg.version, x.pkg.ember.deprecationCompliance);
          deprecationCompliance.addons[x.name] = x.pkg.ember.deprecationCompliance;
        } else {
          console.log('💩', 'deprecation compliance: ', x.name, x.pkg.version, );
        }
      });
      mergedConfig.EmberENV.DEPRECATION_COMPLIANCE = deprecationCompliance;

      console.log(mergedConfig.EmberENV);
      return mergedConfig;
    } else {
  ...
  */
  ...
  discoverAddons() {
    ...
    console.log('====================');
    console.log(`${addonPackageList.length} addons discovered`);
    console.log('====================');
