...
function findupPath(pathName) {
  ...
  if (!pkgPath || isYarnWorkspacke(pkgPath)) {
...
function isYarnWorkspacke(pkgPath) {
  let packageJson = JSON.parse(fs.readFileSync(pkgPath, { encoding: 'utf-8' }));
  return Object.keys(packageJson).includes('workspaces');
}
