...
Project.prototype.availableAddons = function() {
  ...
  this.addIfAddon(path.join(internalMiddlewarePath, 'history-support'), addonPackages);
  ...
  this.discoverAddons(this.root, this.pkg, addonPackages);

  return addonPackages;
};

Project.prototype.discoverAddons = function(root, pkg, addonPackages) {
...
Project.prototype.addIfAddon = function(addonPath, addonPackages) {
  ...
  if(fs.existsSync(pkgPath)) {
    ...
    if (keywords.indexOf('ember-addon') > -1) {
      ...
      this.discoverAddons(addonPath, addonPkg, addonPackages);
    }
  }
};
