...
Project.prototype.config = function(env) {
  var configPath, requirePath;

  if (this.pkg['ember-addon'] && this.pkg['ember-addon']['configPath']) {
    configPath = this.pkg['ember-addon']['configPath'];
    requirePath = './' + path.join(configPath, 'environment');
  } else {
    configPath = path.join(this.root, 'config', env + '.js');
    requirePath = './config/' + env;
  }

  if (fs.existsSync(configPath)) {
    var config = new Config();
    this.require(requirePath)(config);
    return config;
  } else {
    return { };
  }
