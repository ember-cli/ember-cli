  ...
  */
  ...
  config(env) {
    ...
    if (config === undefined) {
      ...
      console.log('found config in cache');
    }
  ...
  configWithoutCache(env) {
    ...
    if (fs.existsSync(`${configPath}.js`)) {
      ...
      deprecationCompliance.self = this.pkg.ember.deprecationComplance;
      deprecationCompliance.addons = {};

      this.addons.forEach((x) => {
        if (x.pkg.ember && x.pkg.ember.deprecationComplance) {
          deprecationCompliance.addons[x.name] = x.pkg.ember.deprecationComplance;
        }
      });
