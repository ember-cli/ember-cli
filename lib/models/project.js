...
class Project {
  ...
  */
  ...
  /**
    ...
    @param  {UI} optionalUI The UI instance to provide to the created Project.
    @return {Project}         Project instance
   */
  static closestSync(pathName, optionalUI, optionalCLI) {
    logger.info('looking for package.json starting at %s', pathName);

    let ui = optionalUI || new UI();
    ...
    if (relative.indexOf('tmp') === 0) {
      ...
      return Project.nullProject(optionalUI, optionalCLI);
    }
    ...
    return new Project(directory, pkg, ui, optionalCLI);
  }
  ...
  /**
    ...
    @return {Project}         Project instance
   */
  static projectOrnullProject(optionalUI, _cli) {
    try {
      return Project.closestSync(process.cwd(), optionalUI, _cli);
    } catch (reason) {
      if (reason instanceof Project.NotFoundError) {
        return Project.nullProject(optionalUI, _cli);
      } else {
        throw reason;
      }
    }
