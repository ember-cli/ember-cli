  ...
  treeFor: function treeFor(name) {
    ...
    if (experiments.ADDON_TREE_CACHING) {
      ...
      if (ADDON_TREE_CACHE[cacheKeyForTreeType]) {
        cacheLogger.info('Cache Hit: ' + this.name + ' - ' + name + ' - ' + cacheKeyForTreeType);
        return ADDON_TREE_CACHE[cacheKeyForTreeType];
      } else {
        cacheLogger.info('Cache Miss: ' + this.name + ' - ' + name + ' - ' + cacheKeyForTreeType);
      }
    }
    ...
    if (cacheKeyForTreeType) {
      cacheLogger.info('Cache Add: ' + this.name + ' - ' + name + ' - ' + cacheKeyForTreeType + ' - ' + !!tree);
...
if (experiments.ADDON_TREE_CACHING) {
  addonProto[experiments.ADDON_TREE_CACHING] = function cacheKeyForTree(treeType) {
    ...
    var cacheKey = crypto.createHash('md5').update(JSON.stringify(cacheKeyParts), 'utf8').digest('hex');
