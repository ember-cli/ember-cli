...
function processModulesOnly(tree) {
  let options = {
    plugins: [
      [require('babel-plugin-transform-es2015-modules-amd'), { loose: true }],
    ],
    moduleIds: true,
  ...
  */
  ...
  treeForAddonTestSupport(tree) {
    ...
    if (registryHasPreprocessor(this.registry, 'js')) {
      ...
      let options = defaultsDeep({}, this[INTERNAL_BABEL_OPTIONS](), {
...
Addon.prototype[BUILD_BABEL_OPTIONS_FOR_PREPROCESSORS] = function() {
  ...
  if (version && semver.satisfies(version, '^5')) {
    ...
    return defaultsDeep({}, this[INTERNAL_BABEL_OPTIONS]());
  }
};

Addon.prototype[INTERNAL_BABEL_OPTIONS] = function() {
  if (!this._cachedInternalBabelOptions) {
    ...
    this._cachedInternalBabelOptions = buildBabelOptionsForTargets(targets);
  }
