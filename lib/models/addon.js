...
function mergeTrees(_trees, options) {
  ...
  if (trees.length === 1) {
    ...
    return upstreamMergeTrees(trees, options);
  }
}

function _mergeTree(inputTree, options) {
  options = options || {};
...
var Addon = CoreObject.extend({
  ...
  _requireBuildPackages: function() {
    ...
    this.mergeTrees = deprecatedAddonFilters(this, 'this.mergeTrees', 'broccoli-merge-trees', _mergeTree);
  ...
  eachAddonInvoke: function eachAddonInvoke(methodName, args) {
    ...
    return this.addons.map(function(addon) {
      ...
      // 'addon-test-support': 'treeForAddonTestSupport',
          ...
          var state = parent._findHost()._treeForState;
        if (!state) {
          console.log('code not find host from.', addon.name, parent.name);
        } else {

            var hasBeenBuilt = state[key];
            console.log(key);
            if (hasBeenBuilt) {
              console.log('hit:', key);
              return;
      ...
      console.log(addon.name, methodName, treeName);
      if (addon[methodName]) {
  ...
  */
  _eachProjectAddonInvoke: function(methodName, args) {
    ...
    return this.project.addons.map(function(addon) {
			if (addon[methodName]) {
				console.log('_eachProjectAddonInvoke', addon.name, methodName);
  ...
  treeFor: function treeFor(name) {
    ...
    if (this.isDevelopingAddon() && this.hintingEnabled() && name === 'app') {
      trees.push(this.jshintAddonTree());
		...
		var trees = trees.filter(Boolean)
  ...
  */
  ...
  treeForAddon: function(tree) {
    ...
    var stylesTree = this.compileStyles(this._treeFor('addon-styles'));
		var trees = [addonTree, stylesTree].filter(Boolean);
  ...
  */
  ...
  _addonTemplateFiles: function addonTemplateFiles(tree) {
    ...
    trees = trees.filter(Boolean);
		if (trees.length === 0) {
			return (this._cachedAddonTemplateFiles = podTemplates);
		} else {
  ...
  */
  compileTemplates: function(tree) {
    ...
    if (this.shouldCompileTemplates()) {
      ...
      console.log(this.name + '#compileTemplates');
  ...
  */
  jshintAddonTree: function() {
    ...
    var addonJs = this.addonJsFiles(addonPath); // TODO: investigate:w
    ...
    var lintTrees = [].concat(lintJsTrees, lintTemplateTrees).filter(Boolean);
		...
		if (lintTrees.length === 0) { return null; }
