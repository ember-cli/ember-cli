...
var logger = heimdallLogger('ember-cli:addon');
var treeCacheLogger = heimdallLogger('ember-cli:addon:tree-cache');
...
var GLOBAL_TREE_FOR_METHOD_METHODS = ['treeFor', '_treeFor', 'treeGenerator'];
var DEFAULT_TREE_FOR_METHOD_METHODS = {
  app: ['treeForApp'],
  addon: [
...
function deprecatedAddonFilters(addon, name, insteadUse, fn) {
  return function(tree, options) {
    var message = `${name} is deprecated, please use ${insteadUse} directly instead  [addon: ${addon.name}]`;
...
var addonProto = {
  ...
  */
  treeFor: function treeFor(treeType) {
    ...
    var trees = this.eachAddonInvoke('treeFor', [treeType]);
    var tree = this._treeFor(treeType);
    ...
    var mergedTreesForType = mergeTrees(trees, {
      overwrite: true,
  ...
  */
  ...
  _fileSystemInfo() {
    ...
    var hasPodTemplates = files.some(file => {
    ...
    var hasJSFiles = files.some(file => file.match(jsMatcher));
    ...
    var hasTemplates = files.some(file => file.match(extensionMatcher));
  ...
  _addonTemplateFiles: function addonTemplateFiles(tree) {
    ...
    if (this._shouldCompilePodTemplates()) {
      var includePatterns = this.registry.extensionsForType('template')
...
function methodsForTreeType(treeType) {
  var treeMethods = DEFAULT_TREE_FOR_METHOD_METHODS[treeType];
...
*/
var addonTreeCacheExperimentPresent = !!experiments.ADDON_TREE_CACHING;
if (addonTreeCacheExperimentPresent) {
  addonProto[experiments.ADDON_TREE_CACHING] = function cacheKeyForTree(treeType) {
    var self = this;
    var methodsToValidate = methodsForTreeType(treeType);

    // determine if treeFor* (or other methods for tree type) overrides for the given tree
    var anyMethodsModified = methodsToValidate.some(methodName => self[methodName] !== addonProto[methodName]);
    ...
    // compute cache key
    var cacheKey = calculateCacheKeyForTree(treeType, self);

    return cacheKey; // profit?
  };
}

var Addon = CoreObject.extend(addonProto);
