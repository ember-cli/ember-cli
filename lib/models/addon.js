...
var cacheLogger = heimdallLogger('ember-cli:addon:tree-cache');
...
var calculateCacheKeyForTree = require('../utilities/addon-calculate-cache-key-for-tree');
  ...
  */
  treeFor: function treeFor(name) {
    ...
    if (experiments.ADDON_TREE_CACHING) {
      cacheKeyForTreeType = this[experiments.ADDON_TREE_CACHING](name);

      if (ADDON_TREE_CACHE[cacheKeyForTreeType]) {
        addonTreeCacheStats.hits++;
        cacheLogger.info('Cache Hit: ' + ' - ' + cacheKeyForTreeType);
        node.stop();
        return ADDON_TREE_CACHE[cacheKeyForTreeType];
      } else {
        addonTreeCacheStats.misses++;
        cacheLogger.info('Cache Miss: ' + ' - ' + cacheKeyForTreeType);
      }
    }
    ...
    if (cacheKeyForTreeType) {
      ...
      cacheLogger.info('Cache Add: ' + ' - ' + cacheKeyForTreeType + ' - ' + !!tree);
...
if (experiments.ADDON_TREE_CACHING) {
  addonProto[experiments.ADDON_TREE_CACHING] = function cacheKeyForTree(treeType) {
    ...
    var anyMethodsModified = methodsToValidate.some(function(methodName) {
      return self[methodName] !== addonProto[methodName];
    });
