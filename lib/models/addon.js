...
let addonProto = {
        ...
        if (parentName !== this.name && !process.env.EMBER_CLI_IGNORE_ADDON_NAME_MISMATCH) {
          ...
          throw new SilentError(
            'Your names in package.json and index.js should match. ' +
              `The addon in ${pathToDisplay} currently have '${parentName}' in package.json and '${
                this.name
              }' in index.js. ` +
  ...
  treeGenerator(dir) {
    ...
    if (!this.project) {
      this._warn(
        `Addon: \`${
          this.name
        }\` is missing addon.project, this may be the result of an addon forgetting to invoke \`super\` in its init.`
      );
    }
    ...
    if (!this.options[emberCLIBabelConfigKey]) {
      this._warn(
        `Ember CLI addons manage their own module transpilation during the \`treeForAddon\` processing. ` +
          `\`${this.name}\` (found at \`${
            this.root
          }\`) has overridden the \`this.options.${emberCLIBabelConfigKey}\` ` +
          `options which conflicts with the addons ability to transpile its \`addon/\` files properly. ` +
          `Falling back to default babel configuration options.`
      );
  ...
  */
  ...
  /**
   ...
   ```js
   importTransforms() {
    let superResult = this._super.importTransforms.apply(this, arguments);
  ...
  */
  importTransforms() {
    ...
    return Object.assign.apply(Object, [{}].concat(transforms));
  },
};
