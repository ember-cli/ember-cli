...
let addonProto = {
  ...
  */
  ...
  treeForAddonTestSupport(tree) {
    ...
    let namespacedTree = this.addonTestSupportFiles(tree);
  ...
  */
  jshintAddonTree() {
    ...
    if (existsSync(appPath)) {
      let appJs = this.appJsFiles(appPath);
      let lintAppJsTrees = this._eachProjectAddonInvoke('lintTree', ['app', appJs]);

      lintTrees = lintTrees.concat(lintAppJsTrees);
    }

    let testSupportPath = this._treePathFor('test-support');
    if (existsSync(testSupportPath)) {
      let lintTestSupportTrees = this._eachProjectAddonInvoke('lintTree', ['test-support', testSupportPath]);

      lintTrees = lintTrees.concat(lintTestSupportTrees);
    }

    let addonTestSupportPath = this._treePathFor('addon-test-support');
    if (existsSync(addonTestSupportPath)) {
      let addonTestSupport = this.addonTestSupportFiles(addonTestSupportPath);
      let lintAddonTestSupportTrees = this._eachProjectAddonInvoke('lintTree', ['addon-test-support', addonTestSupport]);

      lintTrees = lintTrees.concat(lintAddonTestSupportTrees);
    }
  ...
  */
  addonJsFiles(tree) {
    ...
    return new Funnel(tree, {
      include: includePatterns,
      destDir: this.moduleName(),
      annotation: `Funnel: Addon JS (${this.name})`,
    });
  },
  ...
  appJsFiles(tree) {
    ...
    return new Funnel(tree, {
      include: includePatterns,
      annotation: `Funnel: App JS (${this.name})`,
    });
  },

  addonTestSupportFiles(tree) {
    return new Funnel(tree, {
      srcDir: '/',
      destDir: `/${this.moduleName()}/test-support`,
      annotation: `Funnel: Addon Test Support (${this.name})`,
    });
  },
