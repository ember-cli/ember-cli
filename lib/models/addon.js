...
Addon.prototype._requireBuildPackages = function() {
  ...
  // compileES6 is here for compatibility with addons that may be
  // using it. We don't anymore. Also note that we had to explicitly
  // add es6-module-transpiler to our own package.json, because it
  // refuses to load two copies at once, and both compileES6 and
  // tranpsileES6 below need it.
  this.compileES6  = this.compileES6 || require('broccoli-es6-concatenator');

  this.transpileES6  = this.transpileES6 || require('broccoli-es6modules');
...
Addon.prototype.concatFiles = function(tree, options) {
  options.sourceMapsForExtensions = this.app.options.sourcemaps;
...
Addon.prototype.compileAddon = function(tree) {
  ...
  var es6Tree = new this.transpileES6(
