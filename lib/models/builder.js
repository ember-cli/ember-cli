...
class Builder extends CoreObject {
  ...
  readBuildFile() {
    ...
    let defaultBuildFilePath = project.isEmberCLIAddon() ? 'tests/dummy/ember-cli-build.js' : 'ember-cli-build.js';
    let buildFilePath = path.resolve(projectRoot, this.buildFilePath || defaultBuildFilePath);
    // backward compatibility for addons with `ember-cli-build.js` at project root.
    if (!fs.existsSync(buildFilePath)) {
      buildFilePath = findUp.sync('ember-cli-build.js', { cwd: projectRoot });
    }
    ...
    try {
      ...
      err.message = `Could not require '${buildFilePath}': ${err.message}`;
      throw err;
    }

    process.chdir(projectRoot);

    return buildFile({ project });
  }
