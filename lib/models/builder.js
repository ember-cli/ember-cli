...
class Builder extends CoreObject {
   ...
   */
  readBuildFile(path) {
    let node = heimdall.start({ name: 'Read ember-cli-build' });
    let brocfile;

    // Load via new Broccoli 2.3 ESM/Typescript supported build file
    if (isExperimentEnabled('ESM_TYPESCRIPT')) {
      let brocfilePath =
        findUp.sync('ember-cli-build.js', { cwd: path }) || findUp.sync('ember-cli-build.ts', { cwd: path });

      if (!brocfilePath) {
        throw new SilentError('No ember-cli-build.{js|js} found.');
      }

      brocfile = loadBrocfile({ brocfilePath, cwd: path })({ project: this.project });
    } else {
      // Default build loader
      let buildFile = findBuildFile('ember-cli-build.js', path);
      if (buildFile) {
        brocfile = buildFile({project: this.project});
      }
    }
