...
const TARGET_INSTANCE = Symbol.for('_targetInstance_');
...
class PerBundleAddonCache {
  constructor(project) {
    ...
    this.engineAddonDedupeEnabled = !!process.env.EMBER_ENGINES_ADDON_DEDUPE;
   ...
   * empty string for a name, the lazy engines use their engineName, since those are unique)
   ...
   *
   * NOTE: this only applies if this.engineAddonDedupeEnabled is truthy. If it is not, the
  ...
  /**
   * Called by engine-addon when it needs to retrieve the real child addons for a given
   * object (i.e., during ancestorHostAddons), even though the object might be a proxy.
   *
   * @method getAddonAddons
   * @param {Addon|Proxy} addonInstance an instance of an addon or a Proxy generated by this class
   * @return {Array} an array of addon instances
   */
  getAddonAddons(addonInstance) {
    let result =
      addonInstance[TARGET_INSTANCE] && addonInstance[TARGET_INSTANCE].addons
        ? addonInstance[TARGET_INSTANCE].addons
...
// Support for per-bundle addon caching is opt OUT. Caching any particular
// addon class is currently opt IN.
