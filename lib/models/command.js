...
const { union, map, defaults, filter, uniq, uniqBy, reject } = require('ember-cli-lodash-subset');
...
let Command = CoreObject.extend({
  ...
  */
  registerOptions(options) {
    ...
    let optionKeys = uniq(map(this.availableOptions, 'name'));
  ...
  */
  mergeDuplicateOption(key) {
    ...
    if (duplicateOptions.length > 1) {
      ...
      map(duplicateOptions, 'aliases').map((alias) => {
        alias.map((a) => {
