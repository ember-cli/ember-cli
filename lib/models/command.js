  ...
  parseArgs(commandArgs) {
    ...
    this.availableOptions.forEach((option) => {
      ...
      if (
        ...
        (Array.isArray(knownOpts[option.name]) &&
          knownOpts[option.name].includes(String))
      ) {
    ...
    for (let i = 0; i < maskedArgs.length; i++) {
      ...
      if (stringOptions.has(arg) && i + 1 < maskedArgs.length) {
        if (
          typeof maskedArgs[i + 1] === 'string' &&
          maskedArgs[i + 1].startsWith('--')
        ) {
    ...
    let unmask = (value) =>
      typeof value === 'string' && value.startsWith(MASK)
        ? value.replace(MASK, '')
        : value;
