...
const fs = require('fs');
const discoverYarn = require('../utilities/discover-yarn');
    ...
    return new Promise(resolve => {
      ...
      if (this.works === 'insideProject') {
        if (!this.project.hasDependencies()) {
          let installInstruction = '`npm install`';
          if (discoverYarn(this.project.root)) {
            installInstruction = '`yarn install`';
          }
          throw new SilentError(`node_modules appears empty, you may need to run ${installInstruction}`);
        }
      }
