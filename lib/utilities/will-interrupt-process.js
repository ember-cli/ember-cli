...
module.exports = {
  capture(outerProcess) {
    ...
    // Check if `outerProcess` implements the used `EventEmitter` methods.
    // This used to be an `instanceof EventEmitter` check, but that fails when using Bun (https://bun.sh/).
    // See https://github.com/oven-sh/bun/issues/5121 for more info.
    if (['emit', 'off', 'on', 'setMaxListeners'].some((method) => typeof outerProcess?.[method] !== 'function')) {
...
function teardownSignalsTrap() {
  _process.off('SIGINT', exit);
  _process.off('SIGTERM', exit);
...
function cleanupWindowsSignals(_process) {
  ...
  stdin.off('data', windowsCtrlCTrap);
}
