...
function deprecate(description, condition, options) {
  const currentEmbeCLIVersion = process.env.OVERRIDE_DEPRECATION_VERSION ?? emberCLIVersion;
  const isDeprecationRemoved = options.for === 'ember-cli' && semver.gte(currentEmbeCLIVersion, options.until);
