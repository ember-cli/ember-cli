...
var fs = {
  readFile: RSVP.denodeify(require('fs').readFile)
};

module.exports = function bowerInstall() {
  var bower = require('bower');

  return fs.readFile('.bowerrc')
    .then(function(bowerConfig) {
      merge(bowerConfig, { interactive: true });

      return new Promise(function(resolve, reject) {
        bower.commands.install([], { save: true }, bowerConfig) // Packages, options, config
          .on('log', log)
