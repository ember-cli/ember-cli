...
function Command(options) {
  ...
  this.run = function(ui, environment, opts, leek) {
    console.log('leek.track', leek.track.toString())
    if (environment && environment.cliArgs && leek) {
      leek.track(leek.name + ' ' + environment.cliArgs[0] || '', opts && opts.environment ? ', running in ' + opts.environment : '');
      // leek.track(leek.name + ' ' + environment.cliArgs[0] || '', 'Running in ' + opts.environment + ', ' + leek.version);
    }
    options.run.call(self, ui, environment, opts, leek);
  };
