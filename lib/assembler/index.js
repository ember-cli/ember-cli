...
class Assembler {
  ...
  constructor(options) {
    ...
    this._cachedNodeModulesTrees = null;
  }
  ...
  getNodeModulesTrees() {
    if (!this._cachedNodeModulesTrees) {
      this._cachedNodeModulesTrees = Array.from(this._nodeModules.values(), module => new Funnel(module.path, {
        srcDir: '/',
    ...
    return this._cachedNodeModulesTrees;
  }

  /*
    Returns an array of vendor trees from addons
   */
  getAddonVendorTrees() {
    return reduceTreeForByType(this.project.addons, 'vendor');
  }
