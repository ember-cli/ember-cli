...
var outputFile = Promise.denodeify(fs.outputFileSync);
...
Blueprint.prototype.install = function(intoDir, templateVariables, dryRun) {
  ...
  return this.processFiles(intoDir, templateVariables, ui).map(commit);
};
...
Blueprint.prototype.processFiles = function(intoDir, templateVariables) {
  ...
  return Promise.filter(fileInfos, isFile).
    ...
    then(function(infos) {
      ...
      var infosNeedingConfirmation = infos.filter(function(info) {
