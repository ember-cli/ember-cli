...
let mkdirTemp = promisify(temp.mkdir);
...
class InstallBlueprintTask extends Task {
  ...
  async _resolveBlueprint(name = 'app') {
    logger.info(`Resolving blueprint "${name}" ...`);

    try {
      return this._lookupLocalBlueprint(name);
    } catch (error) {
      this._handleLocalLookupFailure(name, error);
    }
  }
