...
module.exports = Task.extend({
  ...
  createHttpsServer: function() {
    if (!existsSync(this.startOptions.sslKey)) {
      throw new TypeError('SSL key couldn\'t be found in "' +
                          this.startOptions.sslKey +
                            '", please provide a path to an existing ssl key file with --ssl-key');
    }

    if (!existsSync(this.startOptions.sslCert)) {
      throw new TypeError('SSL certificate couldn\'t be found in "' +
                          this.startOptions.sslCert +
    ...
    return Promise.resolve()
      ...
      .then(function() {
        return this.listen(options.port, options.host)
          .catch(function() {
            throw new SilentError('Could not serve on http://' +
                                  this.displayHost(options.host) +
                                    ':' +
                                    options.port +
