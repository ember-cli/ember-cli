...
class ExpressServerTask extends Task {
  ...
  processAppMiddlewares(options) {
    if (this.project.has(`${this.serverRoot}.js`) || this.project.has([this.serverRoot, 'index'].join(path.sep))) {
  ...
  restartHttpServer() {
    if (!this.serverRestartPromise) {
      this.serverRestartPromise = this.stopHttpServer()
        .then(() => {
  ...
  startHttpServer() {
    ...
    return Promise.resolve()
      ...
      .then(() =>
        this.listen(options.port, options.host).catch(() => {
