...
module.exports = Task.extend({
  ...
  start: function(options) {
    ...
    this.origin = 'http' + (options.ssl ? 's' : '') + '://' + this.displayHost(options.host) + ':' + options.port + cleanBaseURL(options.baseURL);
    ...
    return this.startHttpServer()
      .then(function () {
        options.ui.writeLine('Serving on ' + this.origin);
      }.bind(this));
  },
    ...
    return Promise.resolve()
      ...
      .then(function(){
        return this.listen(options.port, options.host)
          .catch(function() {
            throw new SilentError('Could not serve on ' + this.origin + '. It is either in use or you do not have permission.');
          }.bind(this));
      }.bind(this));
  },
