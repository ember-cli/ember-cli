...
class BowerInstallTask extends Task {
  ...
  installBower() {
    ...
    this.ui.startProgress(chalk.green('npm: Installing bower ...'));

    return execa('npm', ['install', 'bower@^1.3.12'], { cwd: cliPath })
      .finally(() => this.ui.stopProgress())
      .catch(error => this.handleInstallBowerError(error))
      .then(() => this.ui.writeLine(chalk.green('npm: Installed bower')));
  }
  ...
  ensureBowerJson() {
    ...
    logger.info('Creating "bower.json" for: %s at: %s', projectName, this.bowerJsonPath);

    return writeJson(this.bowerJsonPath, { name: projectName }, { spaces: 2 });
  }
