...
class NpmTask extends Task {
   ...
   * @return {Promise}
   */
  async findPackageManager({ useYarn } = {}) {
    if (useYarn === true) {
  ...
  async run(options) {
    let result = await this.findPackageManager({ useYarn: options.useYarn });
    this.useYarn = Boolean(result.yarnVersion);
