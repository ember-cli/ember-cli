...
const { commands } = require('../utilities/package-managers');
...
class NpmTask extends Task {
  ...
  npm(args) {
    ...
    return commands.npm.invoke(args, { preferLocal: false });
  }

  yarn(args) {
    logger.info('yarn: %j', args);
    return commands.yarn.invoke(args, { preferLocal: false });
  }

  pnpm(args) {
    logger.info('pnpm: %j', args);
    return commands.pnpm.invoke(args, { preferLocal: false });
  }
