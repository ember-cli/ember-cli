...
- Users
  ...
  + Apps now have `ember-disable-proxy-controllers` included by default,
    this ensures that autogenerated controllers always are regular `Ember.Controller`
    instead of the deprecated proxy ones. This does not affect explicitly created
    controllers.
- Addon Developers
  + No changes required
- Core Contributors
